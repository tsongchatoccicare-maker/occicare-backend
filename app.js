...
/* ═══════════════════════════════════════════════════════════════
   PAGE_TO_MOD — Map page name → permission module name
   ─────────────────────────────────────────────────────────────
   Pages whose name != module name (most pages are 1:1 → fallback)
   Used by Router.navigate to check correct permission.
   ═══════════════════════════════════════════════════════════════ */
const PAGE_TO_MOD = {
  calendar: 'dashboard',
  quotation: 'sales',
  op_checklist: 'op_prep',
  op_station_checklist: 'op_checklist',
  op_usage_report: 'op_checklist',
  exam_config: 'config',
  config_checklist: 'config',
  config_stations: 'config',
  config_station_checklist: 'config'
};
// Helper: resolve page → required module (fallback to page itself)
function _modOf(page){ return PAGE_TO_MOD[page] || page; }
...
async editRole(role){
  ...
  Modal.open(html,`แก้ไขสิทธิ์ — ${role}`, async () => {
    // ★ FIX: เริ่มจาก modules เดิม (เก็บ unknown modules ที่ไม่อยู่ใน MODULES dict)
    //    แล้ว update เฉพาะที่อยู่ใน UI form
    const modules = {...(rp.modules||{})};
    Object.keys(MODULES).forEach(k=>{
      modules[k]={};
      actions.forEach(a=>{
        modules[k][a] = document.getElementById(`rp_${k}_${a}`)?.checked || false;
      });
    });
    // Special: qt_approve
    ...
    // ★ FIX: refresh nav ทันที (กรณี admin แก้สิทธิ์ตัวเอง)
    //    User อื่นต้อง logout/login เพื่อเห็น nav ใหม่
    if(typeof buildNav === 'function'){
      const sess = DB.auth.session();
      if(sess && sess.role === role){
        // Admin กำลังแก้สิทธิ์ของ role ตัวเอง → refresh nav
        buildNav();
        U.toast('✅ บันทึกสิทธิ์ + รีเฟรช nav แล้ว','success');
      } else {
        U.toast(`✅ บันทึกสิทธิ์แล้ว — ผู้ใช้ role "${role}" ต้อง logout/login เพื่อใช้สิทธิ์ใหม่`,'success');
      }
    } else {
...
Pages.op_station_checklist = {
  ...
  async render(){
    ...
    // ─── Avatar helper for signer cells ───
    const initial = (s) => U.esc((s||'-').replace(/^(นาย|นาง|น\.ส\.|นางสาว|ดร\.)\s*/,'').charAt(0) || '-');
    const fmtSignD = (iso) => {
      if(!iso) return '';
      try {
    ...
    const signerCell = (name, date, gradColor) => {
      if(!name || !name.trim()){
        return `<div class="opck-sign-empty" title="ยังไม่ลงชื่อ">○</div>`;
      }
      ...
      const tooltip = `${name}${date?' • '+fmtSignFull(date):''}`;
      return `<div class="opck-sign-cell" title="${U.esc(tooltip)}">
        <div class="opck-sign-avatar" style="background:${grad}">${initial(name)}</div>
        ${date?`<div class="opck-sign-date">${fmtSignD(date)}</div>`:''}
      </div>`;
    };
    ...
    document.getElementById('content').innerHTML=`
      ...
      <style>
        .opck-sign-cell{display:inline-flex;flex-direction:column;align-items:center;gap:1px}
        .opck-sign-avatar{width:22px;height:22px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:11px;font-family:'IBM Plex Mono',monospace}
        .opck-sign-date{font-size:9px;color:#6EE7B7;font-family:'IBM Plex Mono',monospace;font-weight:600}
        .opck-sign-empty{display:inline-flex;width:22px;height:22px;border:1px dashed rgba(240,205,127,0.3);border-radius:50%;align-items:center;justify-content:center;color:#9CA3AF;font-size:11px}
      </style>
      <div class="card">
        <div style="padding:12px 16px 0;display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          ...
          <div style="margin-left:auto">
            <button class="btn btn-out btn-xs" onclick="Router.navigate('op_usage_report')">📦 รายงานการใช้ของ</button>
          </div>
        </div>
        <div class="tbl-wrap"><table>
          <thead><tr>
            ...
            <th style="text-align:center" title="ผู้จัดเตรียม">📝 จัด</th>
            <th style="text-align:center" title="ผู้ตรวจสอบ">✓ ตรวจ</th>
            <th style="text-align:center" title="ผู้นำกลับ">↩ กลับ</th>
            <th style="text-align:center" title="ผู้รับคืน">✋ รับ</th>
            <th>สถานะ</th>
            <th></th>
          </tr></thead>
          <tbody>${rows||`<tr><td colspan="10" class="empty"><div class="icon">📋</div><p style="color:#FFFFFF;opacity:.7">ไม่มี Project ที่อยู่ในสถานะ Closed</p></td></tr>`}</tbody>
        </table></div>
      </div>`;
  },
  ...
  openChecklist(pid){
    ...
    // ★ Filter stations by user.station_perms (if set + not admin)
    const sess = DB.auth.session();
    const user = sess ? DB.auth.getUser(sess.userId) : null;
    const isAdmin = sess && sess.role === 'admin';
    const sp = user && Array.isArray(user.station_perms) ? user.station_perms : null;
    const filteredTemplates = (sp && sp.length > 0 && !isAdmin)
      ? Object.fromEntries(Object.entries(templates).filter(([k])=>sp.includes(k)))
      : templates;
...
Pages.op_usage_report = {
  ...
  _availableMonths(){
    ...
    (DB.sales.listProjects()||[]).forEach(p=>{
      if(p.onsite_date) set.add(p.onsite_date.substr(0,7));
    });
